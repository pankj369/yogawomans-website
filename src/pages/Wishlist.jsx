    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#f7ead7_0%,#fdf8f1_100%)] px-4 py-8 text-[#1a1a1a] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 rounded-[2rem] border border-white/60 bg-white/70 p-6 shadow-[0_20px_60px_rgba(72,42,8,0.1)]">
          <p className="text-xs font-bold uppercase tracking-[0.3em] text-[#8b6c48]">Wishlist</p>
          <h1 className="mt-2 text-3xl font-heading font-extrabold">Saved products</h1>
        </div>

        {wishlistItems.length ? (
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {wishlistItems.map((product) => (
              <div key={product.id} className="overflow-hidden rounded-[2rem] border border-white/60 bg-white shadow-[0_20px_60px_rgba(72,42,8,0.1)]">
                <img src={product.image} alt={product.title} className="h-56 w-full object-cover" />
                <div className="space-y-3 p-5">
                  <h2 className="text-xl font-heading font-bold">{product.title}</h2>
                  <p className="text-sm text-[#666]">{product.description}</p>
                  <div className="flex items-center justify-between">
                    <Link to={`/shop/${product.id}`} className="text-sm font-semibold text-[#2E7D32]">
                      View details
                    </Link>
                    <button
                      type="button"
          ...
          <div className="rounded-[2rem] border border-white/60 bg-white/70 p-10 text-center shadow-[0_20px_60px_rgba(72,42,8,0.1)]">
            ...
            <p className="mt-2 text-sm text-[#666]">Save products to revisit them later.</p>
          </div>
        )}
